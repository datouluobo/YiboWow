...
function Addon:CreateSettingsPanel(parent, context)
    ...
    panel:ClearAllPoints(); panel:SetPoint("TOPLEFT", parent, "TOPLEFT"); panel:SetWidth(parent:GetWidth()); panel.rows = panel.rows or {}
    local y, settings = 0, self:GetSettings()
    local heading = panel.heading or Theme:CreateText(panel, Theme.Font.body, Theme.Colors.accent, "LEFT"); panel.heading = heading; heading:ClearAllPoints(); heading:SetPoint("TOPLEFT", 0, -y); heading:SetText("页面与入口"); heading:Show(); y = y + 26
    local note = panel.note or Theme:CreateText(panel, Theme.Font.assist, Theme.Colors.muted, "LEFT"); panel.note = note; note:ClearAllPoints(); note:SetPoint("TOPLEFT", 0, -y); note:SetText("页面显示、独立入口、主表和悬停字段由 Core 的“显示与入口”统一配置。"); note:Show(); y = y + 34
    local restore = panel.restore or Theme:CreateButton(panel, 156, "恢复全部显示", "secondary"); panel.restore = restore; restore:ClearAllPoints(); restore:SetPoint("TOPLEFT", 0, -y); restore:SetScript("OnClick", function() settings.visible = {}; context.notifyPageChanged() end); restore:Show(); y = y + 38
