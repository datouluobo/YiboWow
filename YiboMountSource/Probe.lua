...
local function Print(message)
    if DEFAULT_CHAT_FRAME then
        DEFAULT_CHAT_FRAME:AddMessage("|cff20e070[Yibo] Mount Source:|r " .. message)
    end
end
...
function NS.Probe:Initialize()
    ...
    SLASH_YIBOMOUNTSOURCE1 = "/yms"
    SlashCmdList.YIBOMOUNTSOURCE = function(message)
        ...
        elseif command == "sources" then
            ...
            Print("/yms debug — toggle Aura tooltip diagnostics")
            Print("/yms probe — inspect the hovered tooltip unit and its mount auras")
            Print("/yms inventory — save the client mount spellID inventory for catalog import")
            Print("/yms sources — save client-resolved NPC names for source conversion")
        end
    end
end
