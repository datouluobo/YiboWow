...
- 技术名与插件目录名：`YiboMountSource`
- 插件列表显示名：`|cff20e070[Yibo]|r YiboMountSource - 坐骑来源`
- 目标客户端：World of Warcraft: Mists of Pandaria Classic 5.5.4
- Interface：`50504`
- V1 核心依赖：无
- V1 SavedVariables：无游戏状态；Alpha 的 `/yms inventory` 可选写入一次性诊断快照 `YiboMountSourceDiagnostics`，仅用于生成全量目录基线。
- V1 正式设置命令：无；Alpha 诊断命令：`/yms`
...
`/yms` 只用于 Alpha 技术验证，不保存开关，也不改变玩家看到的 Tooltip 内容：

- `/yms debug`：输出后续 Aura Tooltip 回调中实际取得的 `unit`、`spellID` 和目录命中状态。
- `/yms probe`：检查当前 Tooltip 的相同信息。
...
- 业务页面的承载，不接管 YiboMountSource 的目录和来源数据。
