...
try {
    ...
    # Ship only the icon referenced by the TOC.  Design concepts are useful
    # project assets, but are not runtime files for the CurseForge package.
    $iconSource = Join-Path $projectRoot "Media\\YiboMountSourceIcon-v1.tga"
    if (-not (Test-Path -LiteralPath $iconSource -PathType Leaf)) { throw "Missing addon icon: $iconSource" }
    $iconTarget = Join-Path $curseForgeStage "Media\\YiboMountSourceIcon-v1.tga"
