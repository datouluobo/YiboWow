...
function Get-PackagedAddonVersions {
    param([Parameter(Mandatory = $true)][string]$WorkspaceRoot)
    $versions = [ordered]@{}
    Get-ChildItem -LiteralPath $WorkspaceRoot -Directory -Filter "Yibo*" | Sort-Object Name | ForEach-Object {
        if ($_.Name -eq "YiboCore") { return }
        $tocs = @(Get-ChildItem -LiteralPath $_.FullName -File -Filter "*.toc" | Sort-Object Name)
        if ($tocs.Count -eq 0) { return }
        $tocVersions = @($tocs | ForEach-Object { Get-AddonVersion -TocPath $_.FullName } | Select-Object -Unique)
        if ($tocVersions.Count -ne 1) {
            throw "Inconsistent TOC versions for $($_.Name): $($tocVersions -join ', ')"
        }
        $versions[$_.Name] = $tocVersions[0]
    }
    return $versions
}

function Write-PackagedAddonVersions {
    param(
        [Parameter(Mandatory = $true)][string]$StageAddonRoot,
        [Parameter(Mandatory = $true)][System.Collections.IDictionary]$Versions
    )
    $manifestPath = Join-Path $StageAddonRoot "Data\PackagedAddonVersions.lua"
    $lines = @(
        "-- Generated by Build-ReleasePackages.ps1. Do not commit this staging artifact.",
        "local Core = _G.YiboCore",
        "Core.PackagedAddonVersions = {"
    )
    foreach ($name in $Versions.Keys) {
        $safeName = $name.Replace("\\", "\\\\").Replace('"', '\\"')
        $safeVersion = ([string]$Versions[$name]).Replace("\\", "\\\\").Replace('"', '\\"')
        $lines += ('    ["{0}"] = "{1}",' -f $safeName, $safeVersion)
    }
    $lines += "}"
    Set-Content -LiteralPath $manifestPath -Value $lines -Encoding utf8NoBOM

    $tocPath = Join-Path $StageAddonRoot "YiboCore.toc"
    $tocLines = @(Get-Content -LiteralPath $tocPath)
    if ($tocLines -contains "Data\PackagedAddonVersions.lua") { throw "Staged TOC already contains generated manifest: $tocPath" }
    $insertAt = [Array]::IndexOf($tocLines, "Runtime\AddonStatus.lua")
    if ($insertAt -lt 0) { throw "Unable to find AddonStatus load point in staged TOC: $tocPath" }
    $updated = @($tocLines[0..($insertAt - 1)] + "Data\PackagedAddonVersions.lua" + $tocLines[$insertAt..($tocLines.Count - 1)])
    Set-Content -LiteralPath $tocPath -Value $updated -Encoding utf8NoBOM

    if (-not (Test-Path -LiteralPath $manifestPath -PathType Leaf)) { throw "Generated manifest is missing: $manifestPath" }
    if (-not (Select-String -LiteralPath $tocPath -SimpleMatch "Data\PackagedAddonVersions.lua" -Quiet)) {
        throw "Generated manifest was not added to staged TOC: $tocPath"
    }
}
...
$cfZip = Join-Path $outputRoot ("YiboCore-v{0}-{1}.zip" -f $version, $apiTag)
...
try {
    ...
    $packagedVersions = Get-PackagedAddonVersions -WorkspaceRoot $workspaceRoot
    Write-PackagedAddonVersions -StageAddonRoot (Join-Path $cfStage "YiboCore") -Versions $packagedVersions
    Write-PackagedAddonVersions -StageAddonRoot (Join-Path $githubStage "YiboCore") -Versions $packagedVersions
