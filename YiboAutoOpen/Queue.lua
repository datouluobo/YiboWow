...
function Queue:OnEvent(event, ...)
    ...
    elseif event:find("_CLOSED$") or event == "VOID_STORAGE_CLOSE" then
        ...
        Addon.Safety:SetSensitive(openEvent[event], false); self:RequestScan()
    elseif event:find("UNIT_SPELLCAST") then local unit = ...; if unit == "player" and event ~= "UNIT_SPELLCAST_START" and (not InCombatLockdown or not InCombatLockdown()) then self:RequestScan() end end
end
