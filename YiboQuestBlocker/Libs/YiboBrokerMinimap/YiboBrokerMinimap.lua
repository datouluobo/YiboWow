...
function Adapter:ResetHoverState()
    ...
    self.hoverGraceUntil = nil
    self.hoverCloseToken = (self.hoverCloseToken or 0) + 1
    if self.hoverWatchFrame then
        self.hoverWatchFrame:Hide()
    end
end
...
function Adapter:StartHoverWatcher()
    if not self.hoverWatchFrame then
        ...
        watcher:SetScript("OnUpdate", function(frame, elapsed)
            ...
            if frame.elapsed < 0.08 then
                return
            end
            frame.elapsed = 0
            self:TryCloseTransientHover()
        end)
        self.hoverWatchFrame = watcher
    end
    self.hoverWatchFrame.elapsed = 0
    self.hoverWatchFrame:Show()
end
...
function Adapter:HandleHoverEnter(anchorFrame, isShiftDown)
    if self.hoverOpenedTransient and self.hoverAnchorFrame == anchorFrame then
        self:CancelPendingClose()
        self.hoverGraceUntil = (GetTime and GetTime() or 0) + 0.20
        self:StartHoverWatcher()
        return
    end
    ...
    if self.hoverOpenedTransient then
        self.hoverGraceUntil = (GetTime and GetTime() or 0) + 0.20
        self:StartHoverWatcher()
    end
end
...
function Adapter:TryCloseTransientHover()
    ...
    if self.hoverGraceUntil and (GetTime and GetTime() or 0) < self.hoverGraceUntil then
        return
    end
