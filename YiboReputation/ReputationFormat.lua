...
local function HasFriendProgress(friend)
 local current, min, max = tonumber(friend and friend.current), tonumber(friend and friend.min), tonumber(friend and friend.max)
 return current and min and max and max > min
end
...
function Addon:FormatReputation(data, options)
 ...
 local friend = self:GetFriendship(data)
 if friend then
  if not friend.rank and friend.name == "好友度数据未就绪" and not HasFriendProgress(friend) then return "好友度数据未就绪" end
...
function Addon:FormatCompact(data)
 ...
 local friend = self:GetFriendship(data)
 if friend then
  if not friend.rank and friend.name == "好友度数据未就绪" and not HasFriendProgress(friend) then return "?" end
  if friend.maxRank and friend.rank >= friend.maxRank then return "挚友" end
  local current, min, max = tonumber(friend.current), tonumber(friend.min), tonumber(friend.max)
...
function Addon:FormatMatrix(data, showProgress)
 ...
 local friend = self:GetFriendship(data)
 if friend then
  if not friend.rank and friend.name == "好友度数据未就绪" and not HasFriendProgress(friend) then return "好友度未就绪" end
  local current, min, max = tonumber(friend.current), tonumber(friend.min), tonumber(friend.max)
  if current and min and max and max > min and current - min >= max - min - 1 then return "挚友" end
