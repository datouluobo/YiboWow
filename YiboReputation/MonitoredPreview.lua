...
function Addon:RefreshMonitoredPreview(parent,context)
 ...
 local headerY=y;for ci=0,last-first+1 do local character=ci>0 and context.characters[first+ci-1] or nil;local header=parent.monitoredHeaders[ci+1] or Text(parent,Theme.Font.assist,Theme.Colors.muted,ci==0 and "LEFT" or "CENTER");parent.monitoredHeaders[ci+1]=header;header:ClearAllPoints();header:SetPoint("TOPLEFT",parent,"TOPLEFT",inset.left+(ci==0 and 0 or nameWidth+(ci-1)*cellWidth),-headerY);header:SetWidth(ci==0 and nameWidth or cellWidth);header:SetText(ci==0 and "声望" or CharacterName(character));local color=ci==0 and Theme.Colors.muted or CharacterColor(character);header:SetTextColor(color.r or color[1],color.g or color[2],color.b or color[3]);header:Show() end;for ci=last-first+3,#parent.monitoredHeaders do parent.monitoredHeaders[ci]:Hide() end;local current=Core.Characters:GetCurrent();local currentColumnX;if current then for ci=first,last do if context.characters[ci] and context.characters[ci].id==current.id then currentColumnX=inset.left+nameWidth+(ci-first)*cellWidth;break end end end;y=y+Theme.Size.compact
 for index,id in ipairs(ids) do local row=parent.monitoredRows[index] or CreateFrame("Button",nil,parent,"BackdropTemplate");parent.monitoredRows[index]=row;row:SetSize(nameWidth+(last-first+1)*cellWidth,Theme.Table.previewRowHeight);row:ClearAllPoints();row:SetPoint("TOPLEFT",parent,"TOPLEFT",inset.left,-y);row:SetBackdrop({bgFile="Interface\\Buttons\\WHITE8x8",edgeFile="Interface\\Buttons\\WHITE8x8",edgeSize=1});row:SetBackdropColor((index%2==0 and Theme.Colors.alternate or Theme.Colors.row)[1],(index%2==0 and Theme.Colors.alternate or Theme.Colors.row)[2],(index%2==0 and Theme.Colors.alternate or Theme.Colors.row)[3],1);row:SetBackdropBorderColor(Theme.Colors.matrixLine[1],Theme.Colors.matrixLine[2],Theme.Colors.matrixLine[3],Theme.Colors.matrixLine[4]);row.cells=row.cells or {};row.columnTints=row.columnTints or {};row.columnDividers=row.columnDividers or {};local vals={self:GetFactionName(id)};for ci=first,last do local c=context.characters[ci];local s=Snapshot(c);vals[#vals+1]=self:FormatSnapshotValue(s,Data(c,id),"compact",self:GetFactionState(s,id)) end;local x=0;for ci,v in ipairs(vals) do if ci>1 then local tint=row.columnTints[ci] or row:CreateTexture(nil,"ARTWORK",nil,-2);row.columnTints[ci]=tint;tint:ClearAllPoints();tint:SetPoint("TOPLEFT",row,"TOPLEFT",x,0);tint:SetSize(cellWidth,Theme.Table.previewRowHeight);local tone=(ci%2==0) and Theme.Colors.current or Theme.Colors.selected;tint:SetColorTexture(tone[1],tone[2],tone[3],0.18);tint:Show();local divider=row.columnDividers[ci] or row:CreateTexture(nil,"OVERLAY");row.columnDividers[ci]=divider;divider:ClearAllPoints();divider:SetPoint("TOPLEFT",row,"TOPLEFT",x,0);divider:SetPoint("BOTTOMLEFT",row,"BOTTOMLEFT",x,0);divider:SetWidth(1);divider:SetColorTexture(Theme.Colors.matrixLine[1],Theme.Colors.matrixLine[2],Theme.Colors.matrixLine[3],Theme.Colors.matrixLine[4]);divider:Show() end;local cell=row.cells[ci] or Text(row,Theme.Font.assist,Theme.Colors.text,"LEFT");row.cells[ci]=cell;cell:ClearAllPoints();cell:SetPoint("LEFT",row,"LEFT",x+4,0);cell:SetWidth((ci==1 and nameWidth or cellWidth)-6);cell:SetText(v);if ci>1 then local character=context.characters[first+ci-2];local data=Data(character,id);local color=data and self:GetReputationColor(data) or Theme.Colors.muted;cell:SetTextColor(color[1],color[2],color[3]) else cell:SetTextColor(Theme.Colors.text[1],Theme.Colors.text[2],Theme.Colors.text[3]) end;cell:Show();x=x+(ci==1 and nameWidth or cellWidth) end;row.monitoredTooltip={characters=context.characters,first=first,nameWidth=nameWidth,cellWidth=cellWidth,factionID=id};row.monitoredTooltipCharacterIndex=nil;row:SetScript("OnEnter",function(control) control.monitoredTooltipTracking=true;ShowMonitoredTooltip(control) end);row:SetScript("OnUpdate",function(control) if control.monitoredTooltipTracking then ShowMonitoredTooltip(control) end end);row:SetScript("OnLeave",function(control) control.monitoredTooltipTracking=false;control.monitoredTooltipCharacterIndex=nil;GameTooltip:Hide() end);for ci=#vals+1,#row.cells do row.cells[ci]:Hide() end;for ci=#vals+1,#row.columnTints do row.columnTints[ci]:Hide() end;for ci=#vals+1,#row.columnDividers do row.columnDividers[ci]:Hide() end;row:Show();y=y+Theme.Table.previewRowHeight end
