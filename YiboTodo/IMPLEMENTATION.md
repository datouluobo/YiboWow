# YiboTodo 0.1 - 实施与验收记录

> 文档状态：0.1 已完成。本文件保留 0.1 实施决策、实际交付和验收记录；0.2 及以后的版本范围与新实现决策以 `版本规划.md` 和 `0.2-实施方案.md` 为准。
...
2. `版本规划.md` 决定各版本交付顺序；
3. `IMPLEMENTATION.md` 记录 0.1 如何落到目录、代码、数据、界面和验收，并保留可复用的架构契约与回归依据。
...
- estimated 和异常降级的 unknown 保留与主页面相同的可信度标记。
...
- 超过 24 小时显示日期与时刻；
- 已跨过恢复时间但未重扫显示“预计可用”；
- 未采集冷却但 Core 已确认专业时显示“待确认”，不得显示为“可制作”；unknown 同样用于无法解释的冲突，不显示 `0`、空字符串或“已完成”。
...
### 2026-08-29：合并“专业技能CD”列

- 主表不再分“专业槽位 1 / 2”，两项主专业的已准入冷却合并为单一 `professionProjects` 序列，并显示在“专业技能CD”列。
- 合并列按每个已准入制作项目的产物显示纯图标，不显示“就绪”、剩余时间或专业摘要文字；多项在同一单元格横排，溢出时以 `+N` 收纳。
- 图标状态沿用已确认方案：可制作为全彩，冷却中为灰化加勾，预计可做为全彩加边框，未知为低对比问号；完整状态和时间仅放在悬停中。
- 项目仍保留专业 ID、专业名、来源资料片和公共冷却组；当前只激活 MoP 已验证目录，未来加入其它资料片时不增加永久主表列。
- “通用项目”列改为按实际数据出现；当前没有通用项目时不预留空白宽度。页面自然内容宽度由 `760` 收窄到 `480`，悬停账号预览会直接受益。

### 2026-08-30：MoP 0.1 目录正式接入

- 修正此前“基线已由用户确认、Lua 仍只启用工程和锻造”的断层。炼金活化钢、帝王丝绸、邪煞水晶、智慧卷轴、四个已知珠宝研究法术和神龙之心进入可采集目录；神纹布、两项制皮和两个待索引珠宝研究成员进入可见目录并保持未知，直到补齐 ID。
- 明确排除已知无 API 冷却的“转化：延极锭”，不得把普通无冷却配方显示成每日待办。
- Snapshot 按已确认目录为匹配专业生成 `professionProjects`；有产物 itemID 时使用产物图标，无产物时回退配方图标，缺索引时回退专业图标。未知状态仍按既定方案显示问号，不伪造已确认状态。
- 修复纯图标单元格缺少高度约束的问题；角色行和账号摘要的项目容器现在都有明确垂直尺寸。
- Provider 改为按冷却组合并本次专业窗口观察，不再因打开第二专业而覆盖第一专业缓存。
- 目录版本升至 `6`；业务设置中的冷却组按两栏排列，避免目录扩展后纵向无限拉长。

### 2026-08-30：专业归属默认亮图标与神纹布索引修正（已由后续保守状态策略取代）

- 该版本曾将 Core 已确认的专业归属直接投影为 `actionable`。该策略会在缺少该角色冷却观察时误报可制作，已由后续保守状态策略取代。
- 冷却绝对时间进入 Snapshot 的下一状态边界；到期后重新打开或刷新视图即可自动恢复亮色并显示“按冷却时间”状态，不需要隔天再次扫描。
- Tooltip 简化为项目名、状态，以及冷却中项目的恢复时间，不再重复专业、资料片、周期、目录和最后确认信息。
- 修正 5.5.4 实机确认的神纹布目录：spell `143011`、item `98619`、MoP 来源；修正帝王丝绸产物为 item `82447`。旧 `mop.tailoring.dreamcloth` 设置和观察键在数据库 schema `5` 中迁移到 `mop.tailoring.celestial-cloth`。
- 目录版本升至 `7`，并增加冷却状态冒烟测试，覆盖神纹布/活化钢制作后冷却，以及冷却到期后无需重扫。

### 2026-08-30：国服每日冷却按 07:00 跨日

- 国服服务器日重置是 `07:00`。所有当前启用的 MoP 每日专业冷却组统一改为 `daily-07`；客户端 API 返回的零点式剩余秒数不再阻止跨过 07:00 后恢复亮色。
- 07:00 后状态先显示为“按冷却时间可制作”；重新打开自己的专业面板时，实际 API 观察仍可立即覆盖该推定状态。

### 2026-08-30：专业技能不足状态

- 每个当前 MoP 冷却组在目录中维护自己的专业技能要求；例如帝王丝绸为 550、智慧卷轴为 540、神龙之心为 500。Snapshot 直接使用 Core 已缓存的专业等级，因此即使尚未重新打开专业窗口，也能将低于该项目要求的角色显示为不可制作。
- Provider 仍从自有专业窗口的 `GetTradeSkillInfo` 记录配方可制作性。已确认技能点不足的项目使用独立 `skill-insufficient` 状态：图标低饱和暖色并叠加禁用标记，悬停说明需提升专业技能；它不与冷却中的灰化勾选或预计可做的边框混淆。
