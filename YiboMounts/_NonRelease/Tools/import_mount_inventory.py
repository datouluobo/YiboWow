...
def parse_snapshot(contents: str) -> list[dict[str, object]]:
    ...
    if not records:
        raise ValueError("no mountInventory records found; run /ymt inventory, then fully exit the game before importing")
...
def main() -> int:
    ...
    output = {
        "schemaVersion": 1,
        "source": "YiboMountsDiagnostics.mountInventory",
