...
- Interface：`50504`
- 版本边界（唯一准入规则）：本插件面向的是从经典旧世、燃烧的远征、巫妖王之怒、大地的裂变连续更新到熊猫人之谜 5.5.4 的完整怀旧服分支。凡曾在该分支任一阶段出现、可由该分支角色使用或收藏的坐骑，均属于本插件目录；不得因为坐骑不是 MoP 新增、来源来自更早资料片、活动已结束或当前不可新获取而排除。只有从未进入该怀旧服 5.5.4 分支的正式服/其他分支/未来内容才属于目录外记录。
- V1 核心依赖：无
...
V1 是轻量来源速查工具，不是完整收藏管理器。完整坐骑目录与收藏状态由 YiboCore 页面承载。
...
- 通过只读目录查询与格式化接口为 YiboCore 页面提供业务数据。
...
- 版本准入先按本文件的“版本边界”判定，再判断来源是否已核实；`excluded-*` 只用于确实不属于该连续怀旧服分支的记录，不得用作“当前不可获取”或“不是 MoP 新增”的同义标记。
- 已确认进入该分支的坐骑记录视为既定事实；后续只允许补充或修正来源、名称等证据字段，不得因为来源暂缺、活动结束或当前无法新获取而反复删除、排除再重新加入。
