...
在 MoP Classic 中看到自己或当前目标的坐骑 Aura、聊天坐骑/法术链接或坐骑面板条目，并希望不离开当前界面就知道该坐骑如何获得的玩家。

玩家可能使用暴雪默认单位框架或 NDui。插件不要求玩家安装坐骑收藏插件、YiboCore 或 ALL THE THINGS。

## Product Purpose

在坐骑相关 Tooltip 中追加紧凑、可信且可本地化的获取来源，让玩家直接回答：
...
- 覆盖聊天中的坐骑或法术链接 Tooltip，以及坐骑收藏面板中的坐骑 Tooltip。
...
1. Tooltip 来自 `player` 或 `target` 的 Aura，或来自聊天/坐骑面板且能可靠取得坐骑 `spellID`；
2. 能可靠取得对应的 `spellID`；
...
- 至少一条已收录坐骑聊天/法术链接和一条坐骑面板条目能显示相同来源。
- 未收录 Aura 保持原样。
